    ...
    <>
      <div className="bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-2xl border border-purple-800/30 p-4 hover:border-purple-700/50 transition-all duration-200">
        ...
        {popularCards.length > 0 ? (
          <div className="mb-4">
            <h4 className="text-sm font-semibold text-gray-300 mb-2">
              <Link href="/meta/trending-cards" className="hover:text-purple-400 transition-colors">
                Trending cards
              </Link>
            </h4>
            <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
              {popularCards.map((card) => {
                ...
                const img = cardImages[key] || cardImages[card.name];
                const thumb = img?.art_crop || img?.small || img?.normal;
                return (
                  <button
                    key={card.name}
                    type="button"
                    ...
                    onClick={() =>
                      setSelectedCard({
                        ...
                        imageNormal: img?.normal || img?.art_crop,
                      })
                    }
                    className="group relative h-[4.5rem] w-[3.25rem] shrink-0 overflow-hidden rounded-md border border-purple-700/30 bg-neutral-900 transition hover:border-purple-400/50 hover:shadow-[0_0_12px_rgba(168,85,247,0.2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-400/60"
                  >
                    {thumb ? (
                      <img
                        src={thumb}
                      ...
                      <span className="flex h-full items-center justify-center px-1 text-[9px] font-medium leading-tight text-neutral-500">
                        {card.name.split(" ")[0]}
                      </span>
                    )}
                  </button>
      ...
      {selectedCard && typeof document !== "undefined"
        ? createPortal(
            <WebsiteCardDetailModal
              ...
              imageNormal={selectedCard.imageNormal}
