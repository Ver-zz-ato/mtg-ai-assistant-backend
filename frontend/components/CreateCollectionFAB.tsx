...
export default function CreateCollectionFAB(){
  ...
  async function create(){
    ...
    setBusy(true);
    try{
      const r = await fetch('/api/collections/create', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ name: name.trim() }) });
      const j = await r.json().catch(()=>({})); if(!r.ok || j?.ok===false) throw new Error(j?.error||'Create failed');
      window.location.href = `/collections?collectionId=${encodeURIComponent(j.id || '')}`;
  ...
  return (
    <>
      ...
      <Modal open={open} title="Create new collection" onClose={()=>setOpen(false)}>
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="Collection name" className="w-full bg-neutral-950 border border-neutral-700 rounded px-2 py-1" />
