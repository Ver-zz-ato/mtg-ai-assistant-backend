...
export default function CollectionEditor({ collectionId, mode = "drawer" }: CollectionEditorProps){
  ...
  // Mutations
  async function add(validatedName?: string){
    ...
    const safeName = (validatedName || name).trim();
    ...
    if (!validatedName) {
      ...
      try {
        const validationRes = await fetch('/api/cards/fuzzy', {
          ...
          body: JSON.stringify({ names: [safeName] })
        });
        const validationJson = await validationRes.json().catch(() => ({}));
        const fuzzyResults = validationJson?.results || {};
        
        const suggestion = fuzzyResults[safeName]?.suggestion;
        const allSuggestions = Array.isArray(fuzzyResults[safeName]?.all) ? fuzzyResults[safeName].all : [];
        
        // If name needs fixing, show validation modal
        if (suggestion && suggestion !== safeName && allSuggestions.length > 0) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-10 gap-6" style={{ minHeight:'calc(100vh - 140px)' }}>
      ...
      {selected.size>0 && (
        <div className="fixed bottom-16 right-6 left-6 lg:left-[calc(50%+20px)] lg:right-6 z-20 rounded border border-neutral-800 bg-neutral-950/95 px-4 py-3 flex items-center gap-3 shadow-2xl text-sm">
          ...
          <a href="/wishlist" className="text-sm underline opacity-90 hover:opacity-100">Go to my Wishlist →</a>
