...
export default function DeckCardMenu({ id, title, is_public }: { id:string; title:string|null; is_public:boolean|null }){
  ...
  async function del(){ if(!confirm('Delete this deck?')) return; await fetch('/api/decks/delete', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ id }) }); router.refresh(); setOpen(false); }
