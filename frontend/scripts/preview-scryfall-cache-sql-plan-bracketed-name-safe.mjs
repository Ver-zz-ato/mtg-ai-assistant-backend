#!/usr/bin/env node
/**
 * READ-ONLY: SQL plan for bracketed_name preview (repair_to_canonical only).
 *
 * Reads: tmp/scryfall-cache-preview-bracketed-name.json
 * Queries: scryfall_cache (SELECT only)
 *
 * Skips repairs when the bad PK is not present in DB (not in scope for SQL).
 *
 * Run: npm run preview:scryfall-cache-sql-plan-bracketed-name-safe
 */
...
function parseArgs(argv) {
  let previewJson = join(frontendRoot, "tmp", "scryfall-cache-preview-bracketed-name.json");
  let outSql = join(frontendRoot, "tmp", "scryfall-cache-sql-plan-bracketed-name-safe.sql");
  let outJson = join(frontendRoot, "tmp", "scryfall-cache-sql-plan-bracketed-name-safe.json");
  for (const a of argv) {
...
async function main() {
  ...
  if (!supabaseUrl || !serviceKey) {
    console.error(
      "[bracket-sql-plan] Missing SUPABASE_URL (or NEXT_PUBLIC_SUPABASE_URL) and/or SUPABASE_SERVICE_ROLE_KEY."
    );
    process.exit(1);
  }

  if (!existsSync(previewJson)) {
    console.error(`[bracket-sql-plan] Preview file not found: ${previewJson}`);
    process.exit(1);
  }
  ...
  if (!Array.isArray(allRows)) {
    console.error("[bracket-sql-plan] Expected rows[]");
    process.exit(1);
  }
  ...
  console.log(
    `[bracket-sql-plan] Preview repair rows: ${repairs.length}; fetching ${toFetch.size} PKs...`
  );
  ...
  console.log(`[bracket-sql-plan] Rows fetched from DB: ${dbByName.size}`);
  ...
  const sqlParts = [];
  sqlParts.push(`-- =============================================================================`);
  sqlParts.push(`-- scryfall_cache — bracketed_name safe SQL PREVIEW (not executed)`);
  ...
  console.log("");
  console.log("=== bracketed_name safe SQL plan (preview only) ===");
  ...
  console.log("[bracket-sql-plan] END (ok)");
}

main().catch((err) => {
  console.error("[bracket-sql-plan] FAILED:", err?.message || err);
  process.exit(1);
});
