...
import { isAuthoritativeCommander } from "@/lib/chat/active-deck-context";
...
function wouldInjectAnalyzeBlock(ctx: ActiveDeckContext): boolean {
  return !!ctx.hasDeck && isAuthoritativeCommander(ctx) && !!ctx.commanderName;
}
...
// Inferred commander → confirmation block, no analysis yet
