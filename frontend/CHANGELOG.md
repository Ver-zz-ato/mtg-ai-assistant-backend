...
- **Trending layout:** Guide + meta rotators side-by-side again on desktop; Popular Commander Guides uses responsive grid (12 catalog commanders) on `/new-home`.
- **Density pass:** Popular guides back to one row (6 flagship); community highlights on a single line; public deck count from `/api/decks/browse` total (not meta commander sum); tighter section spacing; tool card subtitles truncated to one line.
- **Hero tool art:** Replaced `/new-home` popular tools banners with new 4:3 assets (`tool-deck-checker.png`, `tool-budget-swaps.png`, `tool-price-tracker.png`, `tool-mulligan-lab.png`, `tool-build-a-deck.png`, `tool-ai-chat.png`). Hero grid **AI Chat** tile links to `/new-chat`.
- **Hero polish:** Removed redundant tool name overlay on art tiles; cyan gradient **Ask ManaTap ✨** CTA; problem-finder tool chips reveal on card hover; embedded Popular Commander Guides header matches trending section typography.
...
- **`/new-chat` polish:** Embedded chat drops left community column (Roast, Shoutbox, Meta Snapshot, Recent Public Decks) and mobile community drawer; signup banner uses compact spacing (no blue ring, tighter margins); FAQ questions use distinct accent colours; intro copy no longer references removed left panel.
- **Sidebar pills (`/new-chat` + shared):** Related tools and explore links render as coloured accent pills; `PopularCommanders` now pulls from external EDHREC daily snapshots (`meta_commander_daily`) instead of blended internal `meta_signals`, with pill styling and freshness line.
- **`/new-chat` sidebar polish:** Popular Commanders compact rotator with art thumbnails; coloured “How it works” / “Use with your deck” headers; embedded chat hides `ModeOptions` bar above signup banner.
