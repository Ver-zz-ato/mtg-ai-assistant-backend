/**
 * getTrendingCards - Trending cards with image map.
...
export async function getTrendingCards(): Promise<{
  items: TrendingCardItem[];
  ...
  const [data, metaRow] = await Promise.all([
    getMetaSignal("trending-cards"),
    supabase
      .from("meta_signals")
      .select("updated_at")
      .eq("signal_type", "trending-cards")
      .maybeSingle(),
  ]);
  ...
  const items: TrendingCardItem[] = raw.map((r, i) => ({
    name: r.name?.trim() ?? "",
    count: r.count ?? 0,
    rank: i + 1,
  }));
