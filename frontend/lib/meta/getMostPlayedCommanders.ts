/**
 * getMostPlayedCommanders - Most played commanders with image map.
...
export type MostPlayedCommanderItem = {
  name: string;
  slug: string;
  count: number;
  rank: number;
...
export async function getMostPlayedCommanders(): Promise<{
  items: MostPlayedCommanderItem[];
  ...
  const [data, metaRow] = await Promise.all([
    ...
    supabase
      .from("meta_signals")
      .select("updated_at")
      .eq("signal_type", "most-played-commanders")
      .maybeSingle(),
  ]);
  ...
  const items: MostPlayedCommanderItem[] = raw.map((r, i) => {
