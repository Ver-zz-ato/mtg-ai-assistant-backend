...
export async function postOpsReportToDiscord(payload: DiscordOpsPayload): Promise<void> {
  ...
  if (payload.reportType === "daily_ops" && payload.dailyDigest) {
    ...
    lines.push(
      ...
      `- OpenAI actual (Costs API, last 24h): $${Number(valueAtPath(digest, ["shared", "revenue", "openai_actual_24h_usd"]) ?? 0).toFixed(2)}`,
      `- OpenAI latest completed UTC day: $${Number(valueAtPath(digest, ["shared", "revenue", "openai_actual_latest_day_usd"]) ?? 0).toFixed(2)} on ${String(valueAtPath(digest, ["shared", "revenue", "openai_actual_latest_day_date_utc"]) || "latest UTC day unavailable")} UTC`,
