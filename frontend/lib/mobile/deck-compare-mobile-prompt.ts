...
export function buildMobileDeckCompareUserPrompt(params: {
  ...
  const schemaHint = [
    ...
    "- Each sections.* array: at most 3 strings; each string under ~150 characters.",
    "- full_analysis values: 1 compact paragraph each, under ~350 characters; avoid rambling.",
    "- Keep the entire JSON compact enough to fit in about 900 output tokens.",
