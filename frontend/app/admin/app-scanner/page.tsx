...
type OverviewPayload = {
  ...
  quality?: {
    ...
    by_source_context?: Record<string, number>;
  };
  aiAssist?: {
...
function BreakdownTable({
  ...
  if (!rows.length) {
    return (
      <div className="rounded-lg border border-neutral-800 p-3">
        <h3 className="mb-2 text-sm font-medium text-neutral-200">{title}</h3>
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-neutral-800">
      <h3 className="border-b border-neutral-800 bg-neutral-900/40 px-3 py-2 text-sm font-medium text-neutral-200">
        {title}
      </h3>
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.key} className="border-t border-neutral-800/80 hover:bg-neutral-900/30">
              <td className="px-3 py-2 font-mono text-amber-200/90">{row.key}</td>
...
export default function AdminAppScannerPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/scanner-analytics/overview?days=${days}`, { cache: "no-store" });
      const json = (await response.json()) as OverviewPayload;
      if (!response.ok || !json.ok) {
        setData(null);
        setErr(json.error || json.hint || "Load failed");
        return;
      }
      setData(json);
    } catch (error: unknown) {
  ...
  const pct = (n: number | null | undefined) => (n == null || Number.isNaN(n) ? "-" : `${n}%`);

  return (
    <div className="mx-auto min-h-screen max-w-6xl space-y-6 bg-neutral-950 p-4 text-neutral-100 md:p-8">
      ...
      <div className="space-y-2 rounded-lg border border-amber-900/50 bg-amber-950/20 px-4 py-3 text-sm text-neutral-300">
      ...
      {err && (
        <div className="whitespace-pre-wrap rounded border border-red-900 bg-red-950/40 px-3 py-2 text-sm text-red-200">
          {err}
        </div>
      )}

      {loading && !overview && !err && <div className="text-sm text-neutral-500">Loading...</div>}
        ...
        <>
          <section>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <Tile label="Scanner starts" value={overview.event_counts?.scan_card_screen_viewed ?? overview.scanner_starts} />
              <Tile label="Scan add initiated" value={overview.scan_add_initiated} sub="Intent (includes non-persist)" />
              <Tile label="Scan add completed" value={overview.scan_add_completed} sub="Persisted add succeeded" />
              <Tile label="Canonical resolution rate" value={pct(overview.canonical_resolution_rate)} sub="% of add initiated" />
              <Tile label="Fail-open rate" value={pct(overview.fail_open_rate)} sub="% of add initiated" />
              <Tile
                label="AI assist usage rate"
                value={pct(overview.ai_assist_usage_rate)}
                sub="% match_source=ai on add initiated"
              />
              <Tile label="AI assist blocked" value={overview.ai_assist_blocked} sub="scan_ai_assist_blocked" />
              <Tile label="Auto-add usage rate" value={pct(overview.auto_add_usage_rate)} sub="% auto_add_enabled true" />
            </div>
            {overview.name_resolution_unaccounted > 0 && (
              <p className="mt-2 text-xs text-neutral-500">
                Add initiated with neither canonical nor fail_open (legacy/missing name_resolution):{" "}
                <span className="font-mono text-neutral-400">{overview.name_resolution_unaccounted}</span>
              </p>
            )}
            {overview.match_pipeline_hint != null && (
              <p className="mt-1 text-xs text-neutral-500">
                AI fallback starts per match completed:{" "}
                <span className="font-mono text-neutral-400">{overview.match_pipeline_hint}%</span> (diagnostic, not
          ...
          <section>
            <h2 className="mb-3 text-lg font-medium text-neutral-200">Funnel (event counts)</h2>
            ...
            <div className="divide-y divide-neutral-800 rounded-lg border border-neutral-800">
              {data?.funnel?.map((step) => (
                <div key={step.id} className="flex flex-wrap items-baseline justify-between gap-2 px-3 py-2">
                  <div>
                    <span className="text-sm text-neutral-200">{step.label}</span>
                    <span className="ml-2 font-mono text-xs text-neutral-500">{step.id}</span>
                  </div>
                  <div className="text-right">
                    <span className="font-mono tabular-nums">{step.count}</span>
                    {step.pct_of_first != null && (
                      <span className="ml-2 text-xs text-neutral-500">{step.pct_of_first}% of first step</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          <section className="grid gap-4 md:grid-cols-3">
            <BreakdownTable title="Add initiated x name_resolution" rows={sortEntries(data?.quality?.by_name_resolution)} />
            <BreakdownTable title="Add initiated x match_source" rows={sortEntries(data?.quality?.by_match_source)} />
            <BreakdownTable
              title="Add initiated x add_confirm_method"
              rows={sortEntries(data?.quality?.by_add_confirm_method)}
            />
            <BreakdownTable title="Add initiated x source_context" rows={sortEntries(data?.quality?.by_source_context)} />
          </section>
          ...
          <section className="space-y-4 rounded-lg border border-neutral-800 p-4">
            ...
            {data?.aiAssist?.blocked_by_reason && Object.keys(data.aiAssist.blocked_by_reason).length > 0 && (
              <BreakdownTable title="Blocked x reason" rows={sortEntries(data.aiAssist.blocked_by_reason)} />
            )}
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded border border-neutral-800 p-3 text-sm">
                <div className="mb-2 text-xs uppercase text-neutral-400">Fallback failed - is_network</div>
              ...
              <div className="overflow-hidden rounded border border-neutral-800">
                <h3 className="border-b border-neutral-800 px-3 py-2 text-xs font-medium uppercase text-neutral-400">
                  Top fallback error strings
                </h3>
                <ul className="max-h-48 divide-y divide-neutral-800 overflow-y-auto text-sm">
                  {(data?.aiAssist?.fallback_failed_top_errors || []).map((row) => (
                    <li key={row.error} className="flex justify-between gap-2 px-3 py-1.5">
                      <span className="mr-2 truncate text-neutral-400" title={row.error}>
                        {row.error}
                      </span>
          ...
          <section className="space-y-3 rounded-lg border border-violet-900/40 bg-violet-950/15 p-4">
