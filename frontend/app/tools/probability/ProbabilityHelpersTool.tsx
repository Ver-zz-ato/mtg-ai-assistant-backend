...
export default function ProbabilityHelpersTool({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  React.useEffect(() => {
    try {
      ...
      // Toggles
      const drawQ = embedded ? null : params.get('draw'); if (drawQ!=null) setOnDraw(drawQ==='1'); else { const d=localStorage.getItem('prob:draw'); if (d!=null) setOnDraw(d==='1'); }
      const xptQ = embedded ? null : params.get('xpt'); if (xptQ!=null && !isNaN(parseInt(xptQ,10))) setExtraPerTurn(parseInt(xptQ,10)); else { const x=localStorage.getItem('prob:xpt'); if (x!=null) setExtraPerTurn(parseInt(x,10)); }
      // Color params
      ['w','u','b','r','g'].forEach((k,idx)=>{ const v=embedded ? null : params.get(k); if(v!=null){ const n=parseInt(v,10); if(Number.isFinite(n)){ [setSrcW,setSrcU,setSrcB,setSrcR,setSrcG][idx](n);} } else { const lsV=localStorage.getItem('prob:'+k); if(lsV!=null){ const n=parseInt(lsV,10); if(Number.isFinite(n)){ [setSrcW,setSrcU,setSrcB,setSrcR,setSrcG][idx](n);} } }});
      ;(['rw','ru','rb','rr','rg'] as const).forEach((k,idx)=>{ const v=embedded ? null : params.get(k); if(v!=null){ const n=parseInt(v,10); if(Number.isFinite(n)){ [setReqW,setReqU,setReqB,setReqR][idx] ? [setReqW,setReqU,setReqB,setReqR,setReqG][idx](n) : setReqG(n); } } else { const lsV=localStorage.getItem('prob:'+k); if(lsV!=null){ const n=parseInt(lsV,10); if(Number.isFinite(n)){ [setReqW,setReqU,setReqB,setReqR][idx] ? [setReqW,setReqU,setReqB,setReqR,setReqG][idx](n) : setReqG(n); } } }});
      const rtQ=embedded ? null : params.get('rt'); if(rtQ!=null){ const n=parseInt(rtQ,10); if(Number.isFinite(n)) setReqTurn(n);} else { const lsrt=localStorage.getItem('prob:rt'); if(lsrt!=null){ const n=parseInt(lsrt,10); if(Number.isFinite(n)) setReqTurn(n);} }
    } catch {}
  }, [embedded]);
  ...
  }, [deckSize, successCards, openingHand, turns, atLeast, onDraw, extraPerTurn, srcW, srcU, srcB, srcR, srcG, reqW, reqU, reqB, reqR, reqG, reqTurn, embedded]);
