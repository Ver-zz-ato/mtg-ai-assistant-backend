...
export async function POST(req: NextRequest) {
  ...
  try {
    let useAdmin = verifyCronRequest(req, {
      routePath: "/api/cron/lightweight-scryfall",
    ...
    if (!useAdmin) {
      logUnauthorizedCronAttempt(req, { routePath: "/api/cron/lightweight-scryfall" });
  ...
  try {
    ...
    const batchSize = parseInt(req.headers.get("x-batch-size") || "150");
    ...
    for (let i = 0; i < cards.length; i += dbBatchSize) {
      const batch = cards.slice(i, i + dbBatchSize);
