...
export async function PUT(req: Request, ctx: { params: Promise<Params> }) {
  ...
  const { data: collection, error: collectionErr } = await supabase
    ...
    .maybeSingle();
  if (collectionErr) {
    return new Response(JSON.stringify({ ok:false, error: collectionErr.message }), { status: 500, headers: { 'content-type':'application/json' } });
  }
  ...
  const { data: existingMeta, error: existingMetaErr } = await supabase
    ...
    .maybeSingle();
  if (existingMetaErr) {
    return new Response(JSON.stringify({ ok:false, error: existingMetaErr.message }), { status: 500, headers: { 'content-type':'application/json' } });
  }
  ...
  if (publicToggleRequested) {
    ...
    if (!cooldown.ok) {
      return new Response(
        JSON.stringify({ ok:false, error: cooldown.message, retryAfterSeconds: cooldown.retryAfterSeconds }),
