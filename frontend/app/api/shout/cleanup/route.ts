...
import { deleteShoutboxMessagesBefore } from "@/lib/server/serviceRoleSupabase";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const { error, count } = await deleteShoutboxMessagesBefore(cutoffISO);

    if (error) {
      console.error("[Shoutbox Cleanup] Delete error:", error);
