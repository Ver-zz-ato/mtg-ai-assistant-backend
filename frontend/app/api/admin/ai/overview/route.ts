...
export async function GET(req: NextRequest) {
  try {
    ...
    const selectCols = 'id,created_at,route,model,request_kind,layer0_mode,context_source,used_two_stage,cache_hit,input_tokens,output_tokens,cost_usd,latency_ms,planner_cost_usd,pricing_version,source_page';
    let q = admin
      .from('ai_usage')
      .select(selectCols)
      .gte('created_at', from)
    ...
    return NextResponse.json({
      ...
      by_source_page: Array.from(by_source_page.entries()).map(([id, t]) => ({ id, ...t, cost_usd: round(t.cost_usd) })).sort((a, b) => b.cost_usd - a.cost_usd),
