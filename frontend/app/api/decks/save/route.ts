...
export async function POST(req: NextRequest) {
  ...
  if (!deckText) return NextResponse.json({ ok: false, error: "deck_text is required" }, { status: 400 });
  ...
  const row = {
    ...
    deck_text: deckText as string,                // legacy column
    data: { cards },                              // JSON for deck page
  ...
  if (error) return NextResponse.json({ ok: false, error: "Insert failed: " + error.message }, { status: 400 });
