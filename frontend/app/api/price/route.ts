...
export async function POST(req: Request) {
  const { names = [], currency = "USD" } = await req.json().catch(() => ({ names: [] }));
  const unique = Array.from(new Set<string>(names)).slice(0, 6);

  // fetch rates once
  const rateRes = await fetch("https://api.exchangerate.host/latest?base=USD&symbols=EUR,GBP");
  ...
  const results = await Promise.all(
    unique.map(async (n) => {
      ...
      const eur = card.prices?.eur ? Number(card.prices.eur) : usd != null ? usd * usdToEur : null;
      const gbp = usd != null ? usd * usdToGbp : eur != null ? eur * (usdToGbp / usdToEur) : null;
      ...
      else if ((card.edhrec_rank ?? 999999) < 2000 && ((usd ?? 0) > 15 || (eur ?? 0) > 15)) risk = "Elevated";
