...
- **Daily ops digest:** `GET /api/cron/ops-report/daily` posts a last-24-hours summary covering app analytics, website activity, **billable LLM calls + internal est. cost** (from `ai_usage`, excludes ai_test/zero-cost rows), revenue, pipeline/Discover job freshness, errors, rate limits, and dashboard watch metrics. Warning-level items such as rate-limit pressure, analytics config gaps, cache reuse, and stale data/jobs stay in the report but are not promoted to launch alerts. `22:30 UTC` matches `23:30` in London during BST; it will arrive an hour earlier in winter unless the cron is adjusted seasonally.
