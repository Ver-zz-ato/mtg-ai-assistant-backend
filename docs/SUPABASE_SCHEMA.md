...
- hosts may update venue/title/Magic format/tournament mode/player cap/Swiss rounds/top cut/pod rounds/round-robin draw setting/pairing mode/deck settings with `PATCH /api/mobile/tournaments/[id]` while the tournament is still in `registration`; setup locks once round one starts
...
- Hosts can manually reseat the current active Commander pod round with `POST /api/mobile/tournaments/[id]/pods/reseat` while all pods are still `pending`; each pod must remain 3-4 players.
- Host start/advance routes accept optional manual 1v1 pairings for Swiss/Round Robin/first elimination rounds, or Commander pod assignments. Top Cut and later elimination bracket rounds advance winners automatically. Swiss rematches require an explicit `allowRematches` confirmation.
