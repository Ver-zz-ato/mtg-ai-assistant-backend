      ...
      <div className="w-full space-y-6 xl:space-y-0 xl:flex xl:gap-6 2xl:gap-8 items-start">
        ...
        <div className="space-y-3 w-full xl:w-[320px] 2xl:w-[340px] shrink-0 relative z-10">
          {deckPreview && (
            <div className="relative overflow-hidden rounded-xl border border-violet-700/60">
              ...
              <div className="relative flex items-center gap-3 p-4">
                {deckPreview.art ? (
                  <img src={deckPreview.art} alt="commander" width={64} height={64} className="rounded-md object-cover border-2 border-emerald-500/50 shadow-lg" />
                ) : (
                  <div className="w-16 h-16 rounded-md bg-neutral-900 border-2 border-neutral-700" />
                )}
                <div className="min-w-0 flex-1">
                  <div className="text-base font-bold truncate text-white drop-shadow-lg">{deckPreview.title || 'Selected deck'}</div>
                  {deckPreview.commander && (
                    <>
                      <div className="text-xs opacity-90 truncate text-white drop-shadow">Commander: {deckPreview.commander}</div>
                      <div className="text-xs opacity-80 italic text-emerald-300 drop-shadow mt-0.5">Let's see what's missing to shuffle for real.</div>
                    </>
                  )}
                </div>
              </div>
          ...
          <div>
            <label className="inline-flex items-center gap-2 text-sm mb-3">
              <input
                ...
                className="rounded border-neutral-600 text-sky-500 focus:ring-sky-500"
              />
          ...
          <button
            onClick={onCompute}
            disabled={busy}
            className={`w-full rounded-md px-4 py-3 font-semibold text-black transition-all ${
              busy 
                ? "bg-sky-300/60 cursor-wait" 
                : "bg-gradient-to-r from-emerald-500 to-sky-500 hover:from-emerald-400 hover:to-sky-400 shadow-lg hover:shadow-emerald-500/50"
            }`}
            style={!busy ? { boxShadow: '0 0 20px rgba(16, 185, 129, 0.3)' } : undefined}
          >
        ...
        <div className="flex-1 min-w-0 w-full max-w-none relative z-20">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-3 text-sm mb-3">
                      <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3">
                        <div className="opacity-70 text-[11px] mb-1">💾 Cards Owned</div>
                        <div className="text-xl font-bold text-emerald-400">{owned}</div>
                      </div>
                      <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3">
                        <div className="opacity-70 text-[11px] mb-1">❌ Missing</div>
                        <div className="text-xl font-bold text-amber-400">{missing}</div>
                      </div>
                      <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3">
                        <div className="opacity-70 text-[11px] mb-1">💸 Biggest</div>
                        <div className="text-base font-bold truncate">{sym}{(biggest?.unit||0).toFixed(0)}</div>
                        <div className="text-[10px] opacity-70 truncate">{biggest?.card || '—'}</div>
                      </div>
                    </div>
                    ...
                    {sparkSvg && (
                      <div className="flex items-center justify-between mt-1 mb-3">
                        <div className="text-[11px] opacity-70">Price trend (last 30 days) {isPro ? '(Pro)' : (<span className="inline-flex items-center gap-1"><span className="px-1.5 py-0.5 rounded bg-amber-300 text-black font-bold uppercase">Pro</span></span>)}</div>
                        {sparkSvg}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        ...
                        <div key={String(label)} className="rounded-lg border border-neutral-800 bg-neutral-900/30 p-2 flex items-center justify-between">
                          <div className="opacity-70 text-[10px]">{label}</div>
                          <div className="text-sm font-semibold">{val as number}</div>
                        </div>
                      ))}
                    </div>
                  </>
          ...
          {/* Top Missing Cards - Enhanced Card Display */}
          {shopItems.length > 0 && (
            <div className="mb-4 w-full max-w-none min-w-0 rounded-xl border border-neutral-800 bg-neutral-950 p-4">
              <div className="text-base font-bold mb-4 flex items-center gap-2">
                <span>🎯 Top Missing Cards</span>
                <span className="text-xs font-normal opacity-70">(highest value)</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {shopItems
                  .slice()
                  .sort((a, b) => Number(b.unit || 0) - Number(a.unit || 0))
                  .slice(0, 6)
                  .map((item, i) => {
                    const key = String(item.name||'').toLowerCase();
                    const img = imgMap[key];
                    const sym = currency==='EUR'?'€':(currency==='GBP'?'£':'$');
                    const rarity = rarityMap[key] || '';
                    const rarityColor = rarity === 'mythic' ? 'text-orange-500' : rarity === 'rare' ? 'text-yellow-500' : rarity === 'uncommon' ? 'text-gray-400' : 'text-neutral-500';
                    
                    return (
                      <div 
                        key={`top-${i}`}
                        className="group relative rounded-lg border border-neutral-800 bg-neutral-900/50 hover:bg-neutral-900 hover:border-neutral-700 transition-all overflow-hidden"
                      >
                        ...
                        <div className="relative h-32 bg-neutral-800 overflow-hidden">
                          ...
                          {/* Price Badge Overlay */}
                          <div className="absolute top-2 right-2 px-2 py-1 rounded-md bg-emerald-600/90 backdrop-blur-sm text-white font-bold text-sm shadow-lg">
                            {sym}{Number(item.unit||0).toFixed(2)}
                          </div>
                        </div>
                        ...
                        <div className="p-3 space-y-2">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className={`${rarityColor} font-medium`}>
                              {rarity ? rarity.charAt(0).toUpperCase() + rarity.slice(1) : 'Common'}
                            </span>
                            <span className="opacity-50">•</span>
                            <span className="opacity-70">Qty: {item.qty}</span>
                          </div>
                          
                          {/* Action Button */}
                          <a
                            href={`/deck/swap-suggestions?cardName=${encodeURIComponent(item.name||'')}`}
                            className="block w-full text-center px-3 py-1.5 rounded bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white text-xs font-semibold transition-all shadow-lg hover:shadow-purple-500/50"
                            target="_blank"
          ...
          {shopItems.length > 0 && (
            <div className="mt-2 w-full max-w-none min-w-0">
              <div className="text-sm font-medium">Full Shopping List</div>
